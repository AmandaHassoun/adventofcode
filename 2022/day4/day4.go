...
func main() {
	// PART 1
	containedRangesCount := getFullyContainedRangeCount("input.txt")
...
func getFullyContainedRangeCount(filename string) (int) {
    count := 0
	...
	for scanner.Scan() {
	    ...
	    contained := checkRanges(strategy[0], strategy[1])
        if contained {
            count++
        }
	}
...
func checkRanges(range1 string, range2 string) (bool) {
